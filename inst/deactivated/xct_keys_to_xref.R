...
xct_format_single <- function(key, bib_obj, key_abort_on_missing = TRUE, ...) {
...
#' @family cite
#' @return Formatted citation as a string.
xct_format <- function(raw_key, bib_obj, ...) {
  ...
  if (grepl("^\\[.*\\]$", raw_key)) {
    ...
    citation <- xct_format_single(key, bib_obj, ...)
...
xct_assemble <- function(citations) {
...
#'
#' We probably just want to use [xct_keys_to_inline()] or [xct_keys_to_inline_table_col()]but this will
...
#' @importFrom cli cli_abort
#' @seealso [xct_keys_to_inline()], [xct_keys_to_inline_table_col()]
#' @export
#' @examples
#' \dontrun{
...
#' }
xct_keys_to_xref <- function(citation_keys, path_bib, key_abort_on_missing = TRUE, key_check_response = "warn") {
  ...
  # Process each citation key through xct_format
  reference_list <- sapply(citation_keys, function(key) {
    xct_format(key, bib_obj, key_abort_on_missing = key_abort_on_missing)
  })
