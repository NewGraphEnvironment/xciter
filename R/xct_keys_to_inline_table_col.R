...
#'
#' This function applies the \link{xct_keys_to_inline} transformation to a specified column
...
#' @export
xct_keys_to_inline_table_col <- function(dat = NULL, col_format = NULL, path_bib = "references.bib") {
  # Validate inputs
  chk::chk_data(dat)
  ...
  dat[[col_format]] <- sapply(dat[[col_format]], function(x) xct_keys_to_inline(x, path_bib = path_bib))
  dat
}
